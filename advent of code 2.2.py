...
def findClose(boxes):
	...
	while ratio < .96:
		...
		idx = (idx + 1) % len(boxes)
		nextItem = boxes[idx]
		ratio = difflib.SequenceMatcher(None, thisItem, nextItem).ratio()
	return thisItem,nextItem

closePair = findClose(sortedList)
# print closePair

# output_list = [li for li in difflib.ndiff(closePair[0],closePair[1]) if li[0] != ' ']
output_list = enumerate(difflib.ndiff(closePair[0],closePair[1]))
print list(output_list)
# print difflib.ndiff(closePair[0],closePair[1])
